...
export default async function KamarPage() {
  const kamar = await prisma.room.findMany({ include: { tenant: true }, orderBy: { number: "asc" } });
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-4">
        ...
        <Link href="/kamar/tambah" className="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700">
          + Tambah Kamar
        </Link>
      </div>
          ...
          <tbody>
            {kamar.map((k) => (
              <tr key={k.id} className="border-t">
                ...
                <td className="p-2">
                  <span className={`px-2 py-1 rounded text-sm ${k.status === "Terisi" ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-800"}`}>
                    {k.status}
                  </span>
                </td>
                <td className="p-2">{k.tenant?.name || "-"}</td>
                <td className="p-2 flex gap-2">
                  <Link href={`/kamar/${k.id}/edit`} className="text-blue-600 hover:underline">Edit</Link>
